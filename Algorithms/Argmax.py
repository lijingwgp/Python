...
# N is the constraint we defined     
    ...
    while low <= high:                                  # the second estimate is more accurate    
        ...
        if abs(guess*log(guess,2) - N) <= error:        # when the difference between our estimation and actual constraint is acceptable
            return (guess, high)
        
        if guess*log(guess,2) < N:                      # if our guess is smaller than constraint, update the lower bound
            low = guess
...
print bisection_search_lgN(2**40)
