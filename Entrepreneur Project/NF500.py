...
def main():
	...
	data['InvestmentVsCategoryAvg'] = np.divide(data['Average FranchiseTot'], data['Average FranchiseTot'].std())
		#data.groupby('rankingCategory').std().loc[data.loc[:,'rankingCategory'],'Average FranchiseTot'].replace(0,0.000001).replace(np.nan,0.000001))
	data['USGrowthVsCategoryAvg'] = np.divide(data['3yrUSGrowth'], data['3yrUSGrowth'].std())
		#data.groupby('rankingCategory').std().loc[data.loc[:,'rankingCategory'],'3yrUSGrowth'].replace(0,0.000001).replace(np.nan,0.000001))
	...
	data = data.replace(-np.inf, 0)
	data = data.fillna(0)
	data['FinanceCheck'] = data['CurrentAssets'] + data['CurrentLiabilities'] + data['TotalAssets']
	...
	data['Pillar4'] = (data['UnitsPerYearP']*UnitsPerYear_w) + (data['SocialScoreP']*SocialScore_w) + (data['TotalUnits2016P']*Totalunits_w) + (data['YrsinBusinessP']*YrsinBusiness_w) + (data['Yrs FranchisingP']*YrsFranchising_w)
	...
	data['Pillar5'] = data.apply(FinancialCheck, axis=1)

	data['FinalScore'] = (data['Pillar1']*p1_w) + (data['Pillar2']*p2_w) + (data['Pillar3']*p3_w) + (data['Pillar4']*p4_w) + (data['Pillar5']*p5_w)
...
def FinancialCheck(row):
	if row['FinanceCheck'] == 0:
		return 0
	else:
		return row['Pillar5']
